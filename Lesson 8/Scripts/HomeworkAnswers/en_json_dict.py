...
def translate(word, eng_2_rus):
    ...
    if len(close_matches) > 0:
        ...
        question = "Возможно вы имели ввиду '{0}' вместо '{1}'? Введите 'Y' чтобы принять: "
        confirm = input(question.format(similar_word, word)).strip()
        if confirm.lower() == "y":
...
def save_dict(p_dict):
    ...
    '''
    with open ('dict.json', 'w', encoding="cp1251") as file:
...
data = json.load(open("dict.json", encoding="cp1251"))
